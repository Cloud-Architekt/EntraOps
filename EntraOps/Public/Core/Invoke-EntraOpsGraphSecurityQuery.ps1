...
.DESCRIPTION
  ...
  It simplifies the process of querying and retrieving data from Microsoft Defender XDR.

.EXAMPLE
  # Get identities with eligible Entra roles of the last 14 days
  Invoke-EntraOpsGraphSecurityQuery -Query "IdentityInfo | where isnotempty(PrivilegedEntraPimRoles)" -Timespan "P14D"
#>
function Invoke-EntraOpsGraphSecurityQuery {
    [CmdletBinding()]
    param(
        # Valid KQL query

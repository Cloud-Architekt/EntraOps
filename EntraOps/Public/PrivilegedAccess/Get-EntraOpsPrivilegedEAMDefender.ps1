...
function Get-EntraOpsPrivilegedEamDefender {
    ...
    if ($UseParallelForClassification) {
        ...
        if ($DefenderRbacClassifiedObjects.Count -ne $UniqueObjects.Count) {
            $WarningMessages.Add([PSCustomObject]@{Type = "Stage-Classification-Parallel"; Message = "Parallel classification returned fewer objects than expected. Expected: $($UniqueObjects.Count), Actual: $($DefenderRbacClassifiedObjects.Count)" })
            Write-Warning "Parallel classification returned fewer objects than expected. Expected: $($UniqueObjects.Count), Actual: $($DefenderRbacClassifiedObjects.Count)"
        }
    } else {
